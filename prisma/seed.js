...
async function createOrUpdateInstitute(data) {
  return prisma.institute.upsert({
    where: { email: data.email ?? `institute-${data.name.replace(/\s+/g, '-').toLowerCase()}@local` },
    create: data,
    update: data,
  });
}

async function createUserIfMissing({ email, role, fullName, instituteId, password }) {
...
async function createBatchIfMissing({ courseId, instituteId, title, startDate, endDate, timezone }) {
